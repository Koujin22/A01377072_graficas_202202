...
THREE.FBXLoader = ( function () {
	...
	FBXTreeParser.prototype = {
			...
			for ( var nodeID in modelNodes ) {
				...
				if ( ! model ) {
					...
					switch ( node.attrType ) {
						...
						case 'camara':
							model = this.createcamara( relationships );
							break;
						case 'Light':
		...
		// create a THREE.Perspectivecamara or THREE.Orthographiccamara
		createcamara: function ( relationships ) {
			...
			relationships.children.forEach( function ( child ) {
				...
				if ( attr !== undefined ) {
					...
					camaraAttribute = attr;

				}

			} );

			if ( camaraAttribute === undefined ) {
				...
				var type = 0;
				if ( camaraAttribute.camaraProjectionType !== undefined && camaraAttribute.camaraProjectionType.value === 1 ) {
				...
				if ( camaraAttribute.NearPlane !== undefined ) {

					nearClippingPlane = camaraAttribute.NearPlane.value / 1000;

				}

				var farClippingPlane = 1000;
				if ( camaraAttribute.FarPlane !== undefined ) {

					farClippingPlane = camaraAttribute.FarPlane.value / 1000;
				...
				if ( camaraAttribute.AspectWidth !== undefined && camaraAttribute.AspectHeight !== undefined ) {

					width = camaraAttribute.AspectWidth.value;
					height = camaraAttribute.AspectHeight.value;
				...
				var fov = 45;
				if ( camaraAttribute.FieldOfView !== undefined ) {

					fov = camaraAttribute.FieldOfView.value;

				}

				var focalLength = camaraAttribute.FocalLength ? camaraAttribute.FocalLength.value : null;

				switch ( type ) {

					case 0: // Perspective
						model = new THREE.Perspectivecamara( fov, aspect, nearClippingPlane, farClippingPlane );
						if ( focalLength !== null ) model.setFocalLength( focalLength );
						break;

					case 1: // Orthographic
						model = new THREE.Orthographiccamara( - width / 2, width / 2, height / 2, - height / 2, nearClippingPlane, farClippingPlane );
						break;

					default:
						console.warn( 'THREE.FBXLoader: Unknown camara type ' + type + '.' );
						model = new THREE.Object3D();
						break;
				...
				children.forEach( function ( child ) {
					...
					if ( child.relationship === 'LookAtProperty' ) {
						...
						if ( 'Lcl_Translation' in lookAtTarget ) {
							...
							} else { // camaras and other Object3Ds
