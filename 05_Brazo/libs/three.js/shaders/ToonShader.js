...
const ToonShader1 = {
	...
	vertexShader: /* glsl */`
		...
		void main() {
			...
			vec3 I = worldPosition.xyz - camaraPosition;
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			float camaraWeighting = dot( normalize( vNormal ), vRefract );
			intensity += pow( 1.0 - length( camaraWeighting ), 6.0 );
...
const ToonShader2 = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			float camara = max( dot( normalize( vNormal ), vec3( 0.0, 0.0, 1.0 ) ), 0.4);
			...
			if ( length(uAmbientLightColor + uDirLightColor * camara) < 0.50 ) {
